...
| **Shopify stock does not update after web sale** | **Outbox Worker Deadlock / Stalled Queue** | `SELECT COUNT(*) FROM outbox_events WHERE status='PENDING'` | Kick the outbox worker: `python -c "from app.workers.shopify_tasks import drain_outbox_events; drain_outbox_events()"` |
